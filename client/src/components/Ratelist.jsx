    ...
    <div className="bg-white text-black min-h-screen p-6">
      ...
      <div className="flex flex-wrap gap-2 justify-center mb-6">
        {categories.map((category, index) => (
          <button
            key={index}
            className={`px-4 py-4 mb-6 flex-grow rounded-lg transition-all duration-300 ${
                selectedCategory === category ? "bg-green-500 text-white" : "bg-gray-200 text-black"
            } hover:bg-green-400`}
              ...
              <div key={index} className="h-fit flex flex-wrap gap-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-5 ">
                  {data.items.map((item, idx) => (
                    <div key={idx} className="bg-gray-100 p-4 rounded-lg h-fit shadow-md">
