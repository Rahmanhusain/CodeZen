...
const OTPhandle = ({ setIsOtpOpen, propemailRef, isforget }) => {
  ...
  const handleChange = async (element, index) => {
    ...
    if (
      ...
      setisvarifying(true);
      const status = isforget
        ? await VerifyOtpforget(newOtp.join(""))
        : await VerifyOtp(newOtp.join(""));
      status === 201
        ? (alert(
            isforget
              ? "Password Reset Successfully,redirecting to Login"
              : "Registration Successful, redirecting to match page"
          ),
          isforget ? Router.push("/login") : Router.push("/profile"))
        : (setisvarifying(false),
          warn("Wrong OTP or might be expire Try Again !"));
      /* setisvarifying(false); */
  ...
  return (
    <div className="fixed top-0 left-0 right-0 min-h-full bg-transparent flex items-center justify-center backdrop-blur-xl">
      <div
        className={`bg-white p-6 rounded-lg shadow-xl max-w-sm w-full flex flex-col items-center m-4 relative ${
      ...
      >
        ...
        {isverified ? (
          ...
          <>
            ...
            <div className="flex items-center gap-2 text-xs leading-3 font-sans px-4 mt-3">
